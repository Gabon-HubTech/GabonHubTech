    ...
    return (
        <nav className="fixed top-0 w-full z-50 bg-tech-dark/80 backdrop-blur-md border-b border-tech-blue/20">
            <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                ...
                {/* --- MENU MOBILE OVERLAY --- */}
                <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${isMobileMenuOpen ? 'pointer-events-auto' : 'pointer-events-none'}`}>

                    {/* Fond d'obscurcissement (Overlay) */}
                    <div
                        className={`absolute inset-0 bg-black/70 transition-opacity duration-300 ${isMobileMenuOpen ? 'opacity-100' : 'opacity-0'}`}
                        onClick={() => setIsMobileMenuOpen(false)}
                    />

                    {/* Panneau latéral OPAQUE */}
                    <div className={`absolute right-0 top-0 h-full w-[80%] max-w-sm bg-[#0a0a0a] border-l border-tech-blue/10 shadow-[-20px_0_40px_rgba(0,0,0,0.8)] transition-transform duration-500 ease-in-out p-10 flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>

                        <div className="flex flex-col space-y-8 mt-24">
                            <p className="text-tech-blue font-mono text-[10px] uppercase tracking-[0.4em] mb-4">Navigation</p>

                            {navLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className={`text-3xl font-bold uppercase tracking-tight transition-colors ${pathname === link.href ? 'text-tech-blue' : 'text-white/90'}`}
                                    onClick={() => setIsMobileMenuOpen(false)}
                                >
                                    {link.name}
                                </Link>
                            ...
                            <div className="pt-8 border-t border-white/5 space-y-6">
                                <p className="text-tech-blue font-mono text-[10px] uppercase tracking-[0.4em]">Expertises</p>

                                <Link href="/events" className="flex items-center space-x-4 text-white/80 hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
                                    <div className="p-3 bg-white/5 rounded-xl text-tech-blue"><Calendar size={22} /></div>
                                    <span className="text-xl font-medium">Événements</span>
                                </Link>

                                <Link href="/formations" className="flex items-center space-x-4 text-white/80 hover:text-white" onClick={() => setIsMobileMenuOpen(false)}>
                                    <div className="p-3 bg-white/5 rounded-xl text-tech-blue"><GraduationCap size={22} /></div>
                                    <span className="text-xl font-medium">HubTech Academy</span>
                                </Link>
                            </div>
                        </div>
                        ...
                        <div className="mt-auto pb-10">
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="w-full py-5 bg-tech-blue text-white text-center font-bold uppercase tracking-widest rounded-2xl shadow-lg shadow-tech-blue/20 block active:scale-95 transition-all"
                            >
                                Rejoindre le Hub
                            </a>
